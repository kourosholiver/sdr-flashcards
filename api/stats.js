// Supabase REST API — no SDK needed, uses native fetch (Node 18+)
...
export default async function handler(req, res) {
  ...
  // If env vars aren't set yet, return empty data gracefully
  if (!SB_URL || !SB_KEY) {
  ...
  if (req.method === 'GET') {
    ...
    const allTime        = {}  // { name: { correct, incorrect } }
    const monthly        = {}  // this month only
    const monthlyHistory = {}  // { 'YYYY-MM': { name: { correct, incorrect } } }
